...
import {
    ...
    FreeScannerSystem,
    FreeScannerTalkgroup,
} from '../freescanner';
import { FreeScannerService } from '../freescanner.service';

@Component({
    selector: 'freescanner-search',
    styleUrls: ['./search.component.scss'],
    templateUrl: './search.component.html',
})
export class FreeScannerSearchComponent implements OnDestroy {
    ...
    playbackList: FreeScannerPlaybackList | undefined;
    ...
    results = new BehaviorSubject(new Array<FreeScannerCall | null>(10));
    ...
    download(id: number): void {
        this.freeScannerService.loadAndDownload(id);
    }

    formChangeHandler(): void {
        if (this.livefeedPlayback) {
            this.freeScannerService.stopPlaybackMode();
        }
    ...
    play(id: number): void {
        this.freeScannerService.loadAndPlay(id);
    }
    ...
    refreshResults(): void {
        ...
        if (!this.callPending && (from >= this.offset + this.limit || from < this.offset)) {
            ...
            const calls: Array<FreeScannerCall | null> = this.playbackList.results.slice(from % this.limit, to % this.limit + 1);
    ...
    searchCalls(): void {
        ...
        const options: FreeScannerSearchOptions = {
            limit: this.limit,
            offset: this.offset,
        ...
        this.freeScannerService.searchCalls(options);
    }
    ...
    private eventHandler(event: FreeScannerEvent): void {
        ...
        if ('livefeedMode' in event) {
            ...
            this.livefeedPlayback = event.livefeedMode === FreeScannerLivefeedMode.Playback;
        }
    ...
    private getSelectedSystem(): FreeScannerSystem | undefined {
    ...
    private getSelectedTalkgroup(): FreeScannerTalkgroup | undefined {
        const system = this.getSelectedSystem();
