...
import {
    ...
    FreeScannerCategoryType,
    FreeScannerConfig,
    FreeScannerEvent,
    FreeScannerLivefeed,
    FreeScannerLivefeedMap,
    FreeScannerLivefeedMode,
    FreeScannerPlaybackList,
    FreeScannerSearchOptions,
} from './freescanner';
...
@Injectable()
export class FreeScannerService implements OnDestroy {
    static LOCAL_STORAGE_KEY_LEGACY = 'freescanner';
    static LOCAL_STORAGE_KEY_LFM = 'freescanner-lfm';
    static LOCAL_STORAGE_KEY_PIN = 'freescanner-pin';

    event = new EventEmitter<FreeScannerEvent>();
    ...
    private call: FreeScannerCall | undefined;
    private callPrevious: FreeScannerCall | undefined;
    private callQueue: FreeScannerCall[] = [];

    private categories: FreeScannerCategory[] = [];
    ...
    private livefeedMap = {} as FreeScannerLivefeedMap;
    private livefeedMapPriorToHoldSystem: FreeScannerLivefeedMap | undefined;
    private livefeedMapPriorToHoldTalkgroup: FreeScannerLivefeedMap | undefined;
    private livefeedMode = FreeScannerLivefeedMode.Offline;
    private livefeedPaused = false;

    private playbackList: FreeScannerPlaybackList | undefined;
    ...
    avoid(options: FreeScannerAvoidOptions = {}): void {
        const clearTimer = (lfm: FreeScannerLivefeed): void => {
            lfm.minutes = undefined;
            lfm.timer?.unsubscribe();
            lfm.timer = undefined;
        };

        const setTimer = (lfm: FreeScannerLivefeed, minutes: number): void => {
            lfm.minutes = minutes;
        ...
        if (this.livefeedMode !== FreeScannerLivefeedMode.Playback) {
            this.cleanQueue();
        }
        ...
        if (this.livefeedMode === FreeScannerLivefeedMode.Online) {
            this.startLivefeed();
        }
    ...
    beep(style = FreeScannerBeepStyle.Activate): Promise<void> {
    ...
    clearPin(): void {
        window?.localStorage.removeItem(FreeScannerService.LOCAL_STORAGE_KEY_PIN);
    }
            ...
            if (this.livefeedMapPriorToHoldSystem) {
                ...
                this.livefeedMap = Object.keys(this.livefeedMap).map((sys) => +sys).reduce((sysMap, sys) => {
                    ...
                    sysMap[sys] = Object.keys(this.livefeedMap[sys]).map((tg) => +tg).reduce((tgMap, tg) => {
                        ...
                        } as FreeScannerLivefeed;

                        return tgMap;
                    }, {} as { [key: number]: FreeScannerLivefeed });

                    return sysMap;
                }, {} as FreeScannerLivefeedMap);
    ...
    isPatched(call: FreeScannerCall): boolean {
    ...
    livefeed(): void {
        ...
        } else if (this.livefeedMode === FreeScannerLivefeedMode.Online) {
            this.stopLivefeed();

        } else if (this.livefeedMode === FreeScannerLivefeedMode.Playback) {
    ...
    loadAndPlay(id: number): void {
        ...
        if (this.livefeedMode === FreeScannerLivefeedMode.Offline) {
            this.livefeedMode = FreeScannerLivefeedMode.Playback;
    ...
    play(call?: FreeScannerCall | undefined): void {
        ...
        const queue = this.livefeedMode === FreeScannerLivefeedMode.Playback
    ...
    queue(call: FreeScannerCall, options?: { priority?: boolean }): void {
        if (!call?.audio || this.livefeedMode === FreeScannerLivefeedMode.Offline) {
            return;
        }
        ...
        if (this.audioSource || this.call || this.livefeedPaused || this.skipDelay) {
            this.event.emit({
                queue: this.livefeedMode === FreeScannerLivefeedMode.Online ? this.callQueue.length : this.getPlaybackQueueCount(),
            });
    ...
    readPin(): string | undefined {
        const pin = window?.localStorage?.getItem(FreeScannerService.LOCAL_STORAGE_KEY_PIN);

        return pin ? window.atob(pin) : undefined;
    }

    savePin(pin: string): void {
        window?.localStorage?.setItem(FreeScannerService.LOCAL_STORAGE_KEY_PIN, window.btoa(pin));
    }

    searchCalls(options: FreeScannerSearchOptions): void {
    ...
    startLivefeed(): void {
        ...
        this.livefeedMode = FreeScannerLivefeedMode.Online;
    ...
    stopLivefeed(): void {
        this.livefeedMode = FreeScannerLivefeedMode.Offline;
    ...
    toggleCategory(category: FreeScannerCategory): void {
        const clearTimer = (lfm: FreeScannerLivefeed): void => {
            lfm.minutes = 0;
            lfm.timer?.unsubscribe();
            lfm.timer = undefined;
        ...
        if (category) {
            ...
            const status = category.status === FreeScannerCategoryStatus.On ? false : true;

            this.config?.systems.forEach((sys) => {
                sys.talkgroups?.forEach((tg) => {
                    const lfm = this.livefeedMap[sys.id][tg.id];

                    if (category.type == FreeScannerCategoryType.Group && tg.group === category.label) {
                        clearTimer(lfm);
                        lfm.active = status;
                    } else if (category.type == FreeScannerCategoryType.Tag && tg.tag === category.label) {
                        clearTimer(lfm);
                        lfm.active = status;
                    }
                });
    ...
    private cleanQueue(): void {
        const isActive = (call: FreeScannerCall) => {
        ...
        this.callQueue = this.callQueue.filter((call: FreeScannerCall) => isActive(call));
    ...
    private download(call: FreeScannerCall): void {
        if (call.audio) {
        ...
        if (Array.isArray(message)) {
            switch (message[0]) {
                case WebsocketCommand.Call:
                    if (message[1] !== null) {
                        const call: FreeScannerCall = message[1];
    ...
    private playbackNextCall(): void {
        if (this.call || this.livefeedMode !== FreeScannerLivefeedMode.Playback || !this.playbackList || this.playbackPending) {
            return;
        }
    ...
    private readLivefeedMap(): void {
        try {
            ...
            let store = window?.localStorage?.getItem(`${FreeScannerService.LOCAL_STORAGE_KEY_LFM}-${this.instanceId}`);

            if (store !== null) {
                lfm = JSON.parse(store);

            } else {
                store = window?.localStorage?.getItem(FreeScannerService.LOCAL_STORAGE_KEY_LEGACY);
            ...
            Object.keys(lfm ?? {}).forEach((sys: string) => {
                Object.keys(lfm[+sys]).forEach((tg) => {
                    ...
                    if (!this.livefeedMap[+sys][+tg]) this.livefeedMap[+sys][+tg] = {} as FreeScannerLivefeed;
    ...
    private rebuildCategories(): void {
        this.categories = Object.keys(this.config.groups || []).map((label) => {
            ...
            const status = allOff ? FreeScannerCategoryStatus.Off : allOn ? FreeScannerCategoryStatus.On : FreeScannerCategoryStatus.Partial;

            return { label, status, type: FreeScannerCategoryType.Group };
        })
        ...
        if (this.config.tagsToggle) {
            this.categories = this.categories.concat(Object.keys(this.config.tags || []).map((label) => {
                ...
                return { label, status, type: FreeScannerCategoryType.Tag };
            }))
        }
        ...
        const lfm = this.config.systems.reduce((sysMap, sys) => {
            sysMap[sys.id] = sys.talkgroups.reduce((tgMap, tg) => {
                ...
                tgMap[tg.id] = (this.livefeedMap[sys.id] && this.livefeedMap[sys.id][tg.id])
                    ...
                    : {
                        active: !(group?.status === FreeScannerCategoryStatus.Off || tag?.status === FreeScannerCategoryStatus.Off),
                    } as FreeScannerLivefeed;

                return tgMap;
            }, sysMap[sys.id] || {} as { [key: number]: FreeScannerLivefeed });
            return sysMap;
        }, {} as FreeScannerLivefeedMap);
    ...
    private saveLivefeedMap(): void {
        ...
        window?.localStorage?.setItem(`${FreeScannerService.LOCAL_STORAGE_KEY_LFM}-${this.instanceId}`, JSON.stringify(lfm));
    }
    ...
    private transformCall(call: FreeScannerCall): FreeScannerCall {
